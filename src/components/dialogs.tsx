...
export const CreateModifyRssFeedDialog = ({
  memory_id,
  fragment_id,
  defaultUrls,
  button,
  ...
  const onSubmit: SubmitHandler<RssFeedInput> = useCallback(
    (data) => {
      ...
      if (fragment_id) {
        // If fragment_id is provided, modify the existing RSS feed
        modifyRssFeed.mutateAsync({ memory_id, fragment_id, urls });
        setIsOpen(false);
      } else {
        addRssToMemory.mutateAsync({ urls, memory_id });
        setIsOpen(false);
      }
    },
